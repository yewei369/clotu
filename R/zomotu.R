...
#' @return a list of preprocessed dataset, reference OTUs and OTU names
#' @examples
...
zomotu<-function(data,thr=0,del=TRUE){ ## analyze zombie OTUs, OTU in (otu*sample)
  ...
  bacs<-as.character(pomus$otu[,1])[which(blood>thr)]
  nyblood<-present(nyda)$blood
  ref<-which(nyblood==1)
  
  if(del) return(list(otu=nyda,ref=ref,bacs=bacs)) else
